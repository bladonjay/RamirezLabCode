...
defsettings.box2.lasermode='Front poke';
            ...
            if size(eventstat,1)>2 && settings.(boxnames{i}).Activate
                if ~strcmpi(lasermode,'none')
                    ...
                    if trigger(end,1)>0
                        ...
                        % find last time he withdrew
                        lastentry=find(trigger(:,1)==0,1,'last');
                        if isempty(lastentry), lastentry=1; end
                        
                        % find out if he withdrew
                        if settings.(boxnames{i}).Withdraw==1
                            didwithdraw=any(find(trigger(lastentry:size(trigger,1),1)==1));
                        else
                            didwithdraw=1;
                        end
                        % if this response ts is greater than cooldown
                        % time, and hes withdrawn recently
                        if  trigger(end)>tmpsession.laserpulses(end)+tmpsettings.CoolDown && didwithdraw
                        ...
                        if trigger(end-1,2)>runthreshold
                            ...
                            if strcmpi(lasermode,'rolly')
                                 output(h.maindsp, sprintf('Rat in box %d withdrew %.2f',i,toc));
                            else
                                output(h.maindsp, sprintf('Rat in box %d stopped running %.2f',i,toc));
                            end
                        end
                    end
                end
...
% gotta kill this
    ...
    function success = saveHist(varargin) 
